 ...
 * Decorator
 *
 * goal: extend the functionality of existing classes but
 *  - without touching those classes (would be against the OPEN-CLOSE-PRINCIPLE)
 *
 *  two ways:
 *  - inheriting from them (not always possible because some classes like String are final)
 *  - stick original class as a member in a decorator class and delegate the functions of the original object
 */
class StringDecorator {
    ...
    public String print777() {
        return "777";
    }

    // delegate methods of existing class, so that the decorator can be used like the original class
    // create delegated methods in IDEA: Alt + Einfg -> Delegate Methods -> select member -> select methods to delegate
    public int length() {
